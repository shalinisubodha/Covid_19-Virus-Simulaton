turtles-own
  [ infected?            ; if true, the turtle is infected
    ...
    my-chance-recover    ; if old? is recover-elderly else chance-recover
    remaining-immunity   ; how many days of immunity the turtle has left
    infected-time        ; how long, in days, the turtle has been infectious
...
to update-display
  ask turtles
    [
      if turtle-shape = "person-age" [ ifelse age > 60 * 365 [ set shape "person service" ][ set shape "person"]]
      set color ifelse-value infected? [ red ] [ ifelse-value immune? [ grey ] [ green ] ] ]
end
...
to recover-or-die
  ifelse old? [ set my-chance-recover recover-elderly ] [ set my-chance-recover recovery-chance ]
  if infected-time > duration                        ;; If the turtle has survived past the virus' duration, then
...
"healthy" 1.0 0 -14439633 true "" "plot count turtles with [ not infected? and not immune? ]"
"not-infected" 1.0 0 -14070903 true "" "plot count turtles with [ immune? or not infected? ]"
